...
const BLOCK_CONFIG: Record<BlockType, any> = {
    heading: { label: '見出し', icon: Type, color: 'border-blue-300 bg-blue-100', textClass: 'text-blue-900 font-bold text-lg', placeholder: '見出しを入力...' },
    text: { label: '本文', icon: Box, color: 'border-emerald-300 bg-emerald-100', textClass: 'text-emerald-900 text-sm', placeholder: 'ここに文章を入力...' },
    image: { label: '画像', icon: ImageIcon, color: 'border-purple-300 bg-purple-100', textClass: 'text-purple-900 font-mono text-xs', placeholder: '画像URL (https://...)' },
    button: { label: 'ボタン', icon: MousePointerClick, color: 'border-orange-300 bg-orange-100', textClass: 'text-orange-900 font-bold text-center', placeholder: 'ボタンの文字' },
    divider: { label: '区切り線', icon: Minus, color: 'border-gray-300 bg-gray-100', textClass: 'hidden', placeholder: '' },
    input: { label: '入力欄', icon: FileInput, color: 'border-cyan-300 bg-cyan-100', textClass: 'text-cyan-900 font-mono text-sm', placeholder: 'プレースホルダー' },
    youtube: { label: '動画', icon: Youtube, color: 'border-red-300 bg-red-100', textClass: 'text-red-900 font-mono text-xs', placeholder: 'YouTube URL' },
    card: { label: 'カード', icon: CreditCard, color: 'border-yellow-300 bg-yellow-100', textClass: '', placeholder: '' }
};
...
const useBlockManager = () => {
    ...
    return { blocks, setBlocks, isDirty, setIsDirty, addBlock, removeBlock, updateBlock, handleDragEnd };
};
...
const SortableBlock = ({ block, onDelete, onChange }: { block: Block, onDelete: (id: string) => void, onChange: (id: string, val: string) => void }) => {
    ...
    const config = BLOCK_CONFIG[block.type] || { label: '不明', icon: Box, color: 'border-gray-300 bg-gray-100', textClass: '', placeholder: '' };
    const Icon = config.icon;

    const parseCardData = (json: string) => { try { return JSON.parse(json); } catch { return { title: '', desc: '', btn: '', img: '' }; } };
        ...
        <div ref={setNodeRef} style={style} className="relative mb-4 pl-8 group"> 
            ...
            <div className={`relative flex items-stretch gap-0 rounded-xl border overflow-hidden transition-all ${config.color} shadow-sm hover:shadow-md`}>
                ...
                <div className="flex-1 p-3 min-w-0">
                    <div className="text-[10px] text-black/50 font-bold tracking-wider mb-1 flex items-center gap-1 uppercase">{config.label}</div>
                    
                    {block.type === 'card' ? (
                        <div className="space-y-2">
                            <input type="text" placeholder="画像URL" className="w-full bg-white/70 rounded px-2 py-1 text-xs text-gray-800 border border-black/10 focus:border-yellow-500 focus:bg-white outline-none" value={parseCardData(block.content).img} onChange={(e) => updateCardData('img', e.target.value)} />
                            <input type="text" placeholder="タイトル" className="w-full bg-white/70 rounded px-2 py-1 text-sm font-bold text-gray-900 border border-black/10 focus:border-yellow-500 focus:bg-white outline-none" value={parseCardData(block.content).title} onChange={(e) => updateCardData('title', e.target.value)} />
                            <textarea placeholder="説明文" rows={2} className="w-full bg-white/70 rounded px-2 py-1 text-xs text-gray-700 border border-black/10 focus:border-yellow-500 focus:bg-white outline-none resize-none" value={parseCardData(block.content).desc} onChange={(e) => updateCardData('desc', e.target.value)} />
                            <input type="text" placeholder="ボタンの文字" className="w-full bg-white/70 rounded px-2 py-1 text-xs text-gray-800 border border-black/10 focus:border-yellow-500 focus:bg-white outline-none" value={parseCardData(block.content).btn} onChange={(e) => updateCardData('btn', e.target.value)} />
                        </div>
                    ) : block.type === 'divider' ? (
                        ...
                        <textarea value={block.content} onChange={(e) => onChange(block.id, e.target.value)} className={`w-full bg-transparent border-none focus:outline-none resize-none h-auto min-h-[24px] ${config.textClass} placeholder:text-black/30`} placeholder={config.placeholder} rows={2} onInput={(e) => { e.currentTarget.style.height = 'auto'; e.currentTarget.style.height = e.currentTarget.scrollHeight + 'px'; }} />
                    ) : (
                        <input type="text" value={block.content} onChange={(e) => onChange(block.id, e.target.value)} className={`w-full bg-transparent border-none focus:outline-none ${config.textClass} placeholder:text-black/30`} placeholder={config.placeholder} />
                    )}
                </div>
...
const BlockRenderer = ({ block }: { block: Block }) => {
    ...
    switch (block.type) {
            ...
            <div className="w-full mb-4 rounded-xl overflow-hidden shadow-sm bg-gray-50">
                {block.content ? (
                    <img 
                        ...
                        onError={(e) => {
                            (e.target as HTMLImageElement).src = "https://placehold.co/600x400/f3f4f6/a1a1aa?text=Image+Error";
                        }}
                    />
                ) : (
        ...
        case 'youtube': {
            ...
            return <div className="w-full aspect-video bg-black rounded-xl overflow-hidden mb-4 shadow-lg">{videoId ? <iframe src={`https://www.youtube.com/embed/${videoId}`} className="w-full h-full" allowFullScreen title="YouTube video" /> : <div className="w-full h-full flex items-center justify-center text-gray-500 bg-gray-100 gap-2"><Youtube size={24}/><span className="text-sm">動画プレビュー</span></div>}</div>;
        }
        case 'card': {
...
export default function CreateBlockPage() {
    ...
    const { blocks, isDirty, setIsDirty, addBlock, removeBlock, updateBlock, handleDragEnd } = useBlockManager();
    ...
    const scrollToolbox = (direction: 'left' | 'right') => {
        toolboxRef.current?.scrollBy({ left: direction === 'left' ? -300 : 300, behavior: 'smooth' });
    };

    const toggleFullScreen = () => {
        if (!document.fullscreenElement) {
            previewWrapperRef.current?.requestFullscreen().catch(err => console.error(err));
        } else {
        ...
        try {
            ...
            if (captureRef.current) {
                try {
                    ...
                    thumbnailBase64 = await toJpeg(captureRef.current, { quality: 0.4, pixelRatio: 1, backgroundColor: '#ffffff', cacheBust: true, skipFonts: true });
                } catch (imgError) {
                    console.warn("サムネイル生成をスキップしました (CORS制約等):", imgError);
                }
            }
        ...
        <div className='h-screen w-full bg-[#F9FAFB] text-gray-900 flex flex-col font-sans overflow-hidden'>
                    ...
                    <div className='h-12 border-b border-gray-100 flex items-center justify-between px-6 bg-white shrink-0'>
                        ...
                        <div className="flex items-center gap-2">
                            <div className="flex items-center bg-gray-100 rounded-lg p-0.5 border border-gray-200">
                                <button onClick={() => setViewMode('mobile')} className={`p-1.5 rounded-md transition-all ${viewMode === 'mobile' ? 'bg-white text-emerald-600 shadow-sm' : 'text-gray-400 hover:text-gray-600'}`} title="モバイル表示"><Smartphone size={14} /></button>
                                <button onClick={() => setViewMode('desktop')} className={`p-1.5 rounded-md transition-all ${viewMode === 'desktop' ? 'bg-white text-emerald-600 shadow-sm' : 'text-gray-400 hover:text-gray-600'}`} title="PC表示"><Monitor size={14} /></button>
                            </div>
            ...
            <div style={{ position: 'fixed', left: '-9999px', top: 0 }}>
                <div ref={captureRef} style={{ width: '800px', height: '600px', background: '#ffffff', padding: '40px' }}>
                    {blocks.map(block => <BlockRenderer key={`capture-${block.id}`} block={block} />)}
                </div>
            </div>

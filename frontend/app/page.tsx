        ...
        <div className="relative mb-0.5 select-none transform scale-90 origin-left">
            ...
            {block.isWrapper && (
                <div className="ml-3 pl-3 border-l-[12px] border-l-inherit min-h-[10px] flex flex-col justify-end relative opacity-80" style={{ borderColor: 'inherit' }}>
                    ...
                    <div className={`relative h-4 w-16 ${styles.bg} rounded-b-sm rounded-tr-sm flex items-center px-2 mt-0.5`}>
                        <TopNotch className={styles.bg} /><BottomNotch className={styles.bg} />
                    </div>
                </div>
            )}
        </div>
...
const PostCard = ({ post }: { post: PostData }) => {
    ...
    return (
        <article onClick={handleClick} className='bg-[#161616] rounded-3xl overflow-hidden mb-8 shadow-xl border border-white/5 hover:border-white/10 transition-colors cursor-pointer group/card'>
            <div className='px-6 py-4 flex items-center justify-between border-b border-white/5 bg-[#1a1a1a]'>
                <div className='flex items-center gap-3'>
                    <div className='w-10 h-10 rounded-full bg-gradient-to-tr from-gray-700 to-gray-600 p-[1px]'>
                         ...
                         <img src={post.userAvatar} alt={post.userName} className='w-full h-full rounded-full bg-black object-cover' onError={(e) => { (e.target as HTMLImageElement).src = 'https://api.dicebear.com/7.x/avataaars/svg?seed=Guest'; }} />
                    </div>
                    <div>
                        <h3 className='font-bold text-sm text-white hover:text-blue-400 transition-colors'>{post.userName}</h3>
                        <div className="flex items-center gap-2">
                            <span className='text-[10px] text-gray-500'>{date}</span>
                            {post.type === 'block' ? (
                                <span className="text-[10px] px-1.5 py-0.5 rounded bg-blue-500/10 text-blue-400 border border-blue-500/20 flex items-center gap-1"><Layers className="w-3 h-3" /> Block</span>
                            ) : (
                                <span className="text-[10px] px-1.5 py-0.5 rounded bg-orange-500/10 text-orange-400 border border-orange-500/20 flex items-center gap-1"><Code2 className="w-3 h-3" /> Text</span>
                            )}
                        </div>
                    </div>
                </div>
                <button className='text-gray-500 hover:text-white p-2 hover:bg-white/5 rounded-full transition-colors'><MoreHorizontal className='w-5 h-5' /></button>
            </div>
            ...
            <div className='flex flex-col md:flex-row h-80 md:h-72 border-b border-white/5'>
                <div className='w-full md:w-1/2 bg-[#0d0d0d] border-b md:border-b-0 md:border-r border-white/5 relative overflow-hidden group'>
                    ...
                    <div className="p-6 pt-10 h-full overflow-auto custom-scrollbar relative">
                        {post.type === 'block' && <div className='absolute inset-0 bg-[radial-gradient(#333_1px,transparent_1px)] [background-size:16px_16px] opacity-20 pointer-events-none'></div>}
                        {post.type === 'text' ? (
                            <pre className='font-mono text-xs text-gray-400 leading-relaxed whitespace-pre-wrap break-all'>{post.code || 'No code content'}</pre>
                        ) : (
                            <div className="pl-2">
                                {post.blocks && post.blocks.length > 0 ? post.blocks.map((block: any) => (<ReadOnlyBlock key={block.id} block={block} />)) : <p className="text-gray-500 text-xs italic">No blocks data.</p>}
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className='w-full md:w-1/2 bg-[#222] relative overflow-hidden group'>
                    <div className="absolute top-3 right-4 text-[10px] font-bold text-white/80 uppercase tracking-widest bg-black/50 backdrop-blur-md px-2 py-1 rounded border border-white/10 z-10">Preview</div>
                    {previewUrl ? (
                        <iframe src={previewUrl} className="w-full h-full border-none opacity-90 transition-opacity duration-700 group-hover:opacity-100" title="Post Preview" sandbox="allow-scripts" style={{ pointerEvents: 'none' }} />
                    ) : (
                    ...
                    <div className="absolute inset-0 flex items-center justify-center bg-black/20 opacity-0 group-hover/card:opacity-100 transition-opacity duration-300">
                        <div className="w-12 h-12 bg-white/10 backdrop-blur-md rounded-full flex items-center justify-center border border-white/20 shadow-lg cursor-pointer hover:bg-white/20 hover:scale-110 transition-all"><Play className="w-5 h-5 text-white fill-current ml-0.5" /></div>
                    </div>
                </div>
            </div>
            ...
            <div className='px-6 py-4 bg-[#161616]'>
                <div className='flex items-center gap-6'>
                    <button onClick={(e) => { e.stopPropagation(); setIsLiked(!isLiked); }} className={`flex items-center gap-2 text-sm font-medium transition-colors group ${isLiked ? 'text-pink-500' : 'text-gray-400 hover:text-white'}`}>
                        <Heart className={`w-5 h-5 transition-transform group-active:scale-75 ${isLiked ? 'fill-current' : ''}`} /><span>{post.likes + (isLiked ? 1 : 0)}</span>
                    </button>
                    <button onClick={(e) => e.stopPropagation()} className='flex items-center gap-2 text-sm font-medium text-gray-400 hover:text-blue-400 transition-colors group'><MessageCircle className='w-5 h-5 group-active:scale-90 transition-transform' /><span>{post.comments}</span></button>
                    <button onClick={(e) => e.stopPropagation()} className='flex items-center gap-2 text-sm font-medium text-gray-400 hover:text-green-400 transition-colors ml-auto'><Share2 className='w-5 h-5' /></button>
                </div>
            </div>
        </article>
    );
        ...
        <div className='flex min-h-screen bg-black text-white font-sans'>
            ...
            <main className='flex-1 md:ml-64 bg-[#0a0a0a] min-h-screen'>
                ...
                <div className='max-w-3xl mx-auto px-4 pb-20'>
                    <div className="md:hidden flex items-center justify-between mb-6 px-2"><span className='text-xl font-bold bg-gradient-to-r from-blue-400 to-cyan-400 bg-clip-text text-transparent'>ScriptShot</span></div>
                    {loading ? (
                        ...
                        <div className='space-y-6'>{posts.map((post) => (<PostCard key={post.id} post={post} />))}</div>
                    ) : (
